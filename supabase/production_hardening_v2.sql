-- LOUTRIS production hardening v2
-- Apply AFTER supabase/schema.sql.
-- Do NOT apply production_hardening.sql; this v2 supersedes that draft.

begin;

-- Browser roles cannot directly mutate authoritative profile/economy fields.
...
-- Public lookup intentionally excludes the private data JSON blob.
...
-- Active participant marker gives us an immutable partial-index predicate.
...
create index if not exists match_sessions_active_idx
  on public.match_sessions(active, ranked, last_action_at);
create index if not exists match_participants_user_active_idx
  on public.match_participants(user_id, active);
...
-- Central settlement. Both profiles are locked in deterministic UUID order,
-- preventing the A->B / B->A deadlock pattern.
...
-- One active ranked match per account, with stale matches rejected.
...
begin
  ...
  return jsonb_build_object('id',v_match.id,'mode','ranked','ranked',true,'length',v_match.length,
    'status',v_match.status,'current_player',v_match.current_player,'turn_no',v_match.turn_no,
...
-- Ordered settlement is reused for guesses.
...
begin
  ...
  where m.id=p_match_id and m.active=true
  ...
  if not exists(select 1 from public.game_words where length=v_match.length and word=v_guess) then
    return jsonb_build_object('ok',false,'error','WORD_NOT_ALLOWED');
  end if;
  ...
  if exists(select 1 from public.match_guesses where match_id=p_match_id and word=v_guess) then
  ...
  if v_guess=v_match.answer then
    ...
    perform public.settle_ranked_match(p_match_id,v_winner,'win');
...
-- Reading match state never returns the answer.
...
begin
  if v_user is null then return jsonb_build_object('ok',false,'error','AUTH_REQUIRED'); end if;
  select m.* into v_match from public.match_sessions m
  where m.id=p_match_id and exists(select 1 from public.match_participants p where p.match_id=m.id and p.user_id=v_user)
  for update;
  if not found then return jsonb_build_object('ok',false,'error','MATCH_NOT_FOUND'); end if;
...
-- Server-side purchase API. Populate shop_catalog from the canonical shop
-- list before enabling purchases. The browser cannot choose the price.
...
-- Restrict future public functions by default; explicit grants above are the allow-list.

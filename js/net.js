  ...
  function findMatch(opts, onMatch) {
    ...
    if (bcOk && foundPeer) {
      ...
      var fallbackTimer = setTimeout(function () {
        if (confirmHandler) off("match:confirmed", confirmHandler);
        if (settled) return;
        settled = true;
      ...
      confirmHandler = function (msg) {
        if (msg.to === clientId && msg.roomId === roomId) {
          off("match:confirmed", confirmHandler);
          if (settled) return;
          settled = true;
      ...
      on("match:confirmed", confirmHandler);
      return;
    }
    ...
    var fallbackHost = setTimeout(function () {
      off("match:propose", propHandler);
      if (settledHost) return;
      settledHost = true;
      send("match:stopseek", {});

...
(function (global) {
  ...
  function open(cfg) {
    ...
    var state = {
      ...
      peerSynced: cfg.opponent && cfg.opponent.isPeer
    };
    ...
    var matchStage = UI.el("div", { class: "match-stage" + (isSolo ? " solo" : "") });
      ...
      if (isDuel) {
        ...
        var rowFlip = match.currentTurn === Game.P2 ? 1 : 0;
        myBoard.node.querySelectorAll(".row").forEach(function (rowEl, r) {
          // P1: 4 chances, P2: 3 — the final (7th) row always belongs to P1.
          var owner = (r === state.match.maxRows - 1) ? "P1" : (((r + rowFlip) % 2 === 0) ? "P1" : "P2");
          rowEl.setAttribute("data-owner", owner);
        });
      }
  ...
  function updateTurnIndicator(state) {
    ...
    if (duel.currentTurn === state.myId) {
      state.turnIndicator.className = "turn-indicator " + activeOwner;
      state.turnIndicator.appendChild(UI.el("div", { class: "ti-lbl", text: "YOUR TURN" }));
    } else {
      state.turnIndicator.className = "turn-indicator " + activeOwner;
  ...
  function broadcastGuess(state, word) {
    if (state.peerSynced && state.cfg.roomId) {
      Net.matchMsg(state.cfg.roomId, "guess", { word: word });
    }
  }

  function showReconnect(state) {
    if (state.ended) return;
    var v = UI.el("div", { class: "rc-veil" }, [UI.el("div", { class: "loader-box" }, [UI.el("div", { class: "spinner" }), UI.el("div", { class: "lt", text: "OPPONENT DISCONNECTED" }), UI.el("div", { class: "ls", text: "Attempting reconnect..." }), UI.button("CLAIM VICTORY", { primary: true, onclick: function () { v.remove(); if (Game.isDuel(state.match)) { state.match.winner = state.myId; } state.match.status = "won"; endMatch(state); } })])]);
  ...
  function finalizeMatch(state, playerWon, draw, guesses, perfect) {
    ...
    var eloDelta = 0;
    if (state.cfg.mode === "multiplayer" && state.opp) {
      if (draw) eloDelta = 0;
      else eloDelta = playerWon ? 30 : -30;
      Store.patch(function (s) { s.ranked.elo = Math.max(0, s.ranked.elo + eloDelta); if (s.ranked.elo > s.ranked.peakElo) s.ranked.peakElo = s.ranked.elo; });
    }
    ...
    if (state.peerSynced && state.cfg.roomId) Net.matchMsg(state.cfg.roomId, "guess", { word: state.match.answer, final: true });

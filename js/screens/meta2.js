...
(function (global) {
  ...
  // ============ LEADERBOARD ============
  var LB_TABS = ["Global", "Country", "Peak", "Wins", "XP", "Season", "Friends"];
  var LB_VIEW = { Global: "global", Country: "country", Peak: "peak", Wins: "wins", XP: "xp", Season: "season", Friends: "global" };
  function leaderboard(initialTab) {
    ...
    function paintTab(t, rows) {
      if (!rows.length) {
        body.appendChild(UI.el("div", { class: "center-stage", style: { "padding": "60px 20px" } }, [
          ...
          UI.el("div", { class: "ls", text: t === "Friends" ? "Open the Friends tab to add someone by username." : "Play a ranked match to appear on the ladder." })
        ]));
        return;
      }
      var me = rows.filter(function (row) { return row.me; })[0] || rows[0];
      var isCountry = t === "Country";
      ...
      var hero = UI.el("div", { class: "lb-hero" }, [
        ...
        UI.el("div", { class: "lb-hero-main", style: { "flex": "1" } }, [
          ...
          UI.el("div", { class: "lb-hero-elo" }, [UI.el("span", { class: "lb-hero-elo-n", text: String(isCountry ? me.totalElo : me.elo) }), UI.el("span", { class: "lb-hero-elo-l", text: isCountry ? "TOTAL ELO" : "ELO" })])
        ])
      ]);
      ...
      rows.forEach(function (r) {
        ...
        var sub = "";
        if (isCountry) sub = r.players + " players · top " + r.topElo;
        else if (t === "Peak") sub = "PEAK " + r.peakElo;
        else if (t === "Wins") sub = r.wins + " WINS";
        else if (t === "XP") sub = r.xp.toLocaleString() + " XP";
        else if (t === "Season") sub = "SEASON " + r.seasonElo;
        list.appendChild(UI.el("div", { class: cls }, [
          UI.el("div", { class: "lb-item-pos", html: r.pos === 1 ? UI.ICON.crown : "#" + r.pos }),
          UI.el("div", { class: "lb-item-av" }, [UI.el("div", { class: "in", text: isCountry ? r.country : r.avatar })]),
          UI.el("div", { class: "lb-item-main" }, [
            UI.el("div", { class: "lb-item-name" }, [UI.el("span", { text: isCountry ? r.country : r.name }), r.me ? UI.el("span", { class: "you", text: "YOU" }) : null]),
            sub ? UI.el("div", { class: "lb-item-sub", text: sub }) : null
          ]),
          UI.el("div", { class: "lb-item-elo" }, [UI.el("span", { class: "lb-item-elo-n", text: String(isCountry ? r.totalElo : r.elo) }), UI.el("span", { class: "lb-item-elo-l", text: isCountry ? "ELO" : "ELO" })])
        ]));
      });
    ...
    function paintOffline() {
      body.appendChild(UI.el("div", { class: "center-stage", style: { "padding": "60px 20px" } }, [
        UI.el("div", { class: "e-ico", html: UI.ICON.crown }),
        UI.el("div", { class: "lt", text: "OFFLINE" }),
        UI.el("div", { class: "ls", text: "Reconnect to load the real ladder." })
      ]));
    }
    ...
    function renderTab(t) {
      UI.clear(body);
      var st = Store.get();
      if (!global.Supabase || !Supabase.configured || !Supabase.isReady()) { paintOffline(); return; }
      var myName = (st.profile.name || "").toUpperCase();
      var view = LB_VIEW[t];
      var load = UI.el("div", { class: "center-stage", style: { "padding": "60px 20px" } }, [
        UI.el("div", { class: "e-ico", html: UI.ICON.crown }),
        UI.el("div", { class: "mute", text: "Loading ladder…" })
      ]);
      body.appendChild(load);
      var finish = function (rows) {
        UI.clear(body);
        if (rows && rows.length) paintTab(t, rows);
        else paintTab(t, []);
      };
      Supabase.leaderboard(view, function (rows) {
        if (!rows || !rows.length) { finish([]); return; }
        if (t === "Friends") {
          var friendNames = {};
          (st.friends || []).forEach(function (f) { friendNames[(f.name || "").toUpperCase()] = true; });
          rows = rows.filter(function (r) { return (r.name || "").toUpperCase() === myName || friendNames[(r.name || "").toUpperCase()]; });
          finish(rows);
          return;
        }
        rows.forEach(function (r) { if ((r.name || "").toUpperCase() === myName) r.me = true; });
        if (view === "country") { finish(rows); return; }
        var hasMe = rows.some(function (r) { return r.me; });
        if (hasMe) { finish(rows); return; }
        Supabase.myStanding(view, function (mine) {
          if (mine && mine.elo > 0 && mine.name) rows.push(mine);
          finish(rows);
        });
      });
    }
  ...
  // ============ FRIENDS ============
  var lastCloudFetch = 0;
  function syncCloudFriends(force, done) {
    // merge cloud friend requests into the local store, then re-render once
    var fetched = false;
    if (!global.Supabase || !Supabase.configured || !Supabase.isReady() || !Supabase.isAuthed()) { if (done) done(fetched); return; }
    var now = Date.now();
    if (!force && now - lastCloudFetch < 2500) { if (done) done(fetched); return; }
    lastCloudFetch = now;
    Supabase.listFriendRequests(function (rows) {
      if (!rows) { if (done) done(fetched); return; }
      fetched = true;
      Store.patch(function (st) {
        rows.forEach(function (r) {
          if (r.status === "accepted") {
            if (!st.friends.some(function (f) { return f.name === r.name; })) {
              st.friends.push({ name: r.name, status: "offline", elo: r.elo, avatar: r.avatar || r.name.slice(0, 2) });
            }
            st.friendOutgoing = st.friendOutgoing.filter(function (x) { return x.name !== r.name; });
            st.friendIncoming = st.friendIncoming.filter(function (x) { return x.name !== r.name; });
          } else if (r.status === "pending" && r.incoming) {
            var inc = st.friendIncoming.filter(function (x) { return x.name === r.name; })[0];
            if (inc) { inc.reqId = r.id; inc.cloud = true; inc.elo = r.elo; inc.avatar = r.avatar; }
            else st.friendIncoming.push({ id: "c" + r.id, name: r.name, from: r.otherId, fromName: r.name, reqId: r.id, cloud: true, elo: r.elo, avatar: r.avatar, when: Date.now() });
          } else if (r.status === "pending" && !r.incoming) {
            var out = st.friendOutgoing.filter(function (x) { return x.name === r.name; })[0];
            if (out) { out.reqId = r.id; out.cloud = true; out.elo = r.elo; out.avatar = r.avatar; }
            else st.friendOutgoing.push({ id: "c" + r.id, name: r.name, reqId: r.id, cloud: true, elo: r.elo, avatar: r.avatar, when: Date.now() });
          }
        });
      });
  ...
  function friends() {
    ...
    var top = UI.el("div", { class: "flex gap12", style: { "margin-bottom": "16px" } }, [
      ...
      UI.button("ADD", { primary: true, onclick: function () {
        ...
        if (!name) { UI.toast("Enter a username.", "error"); Audio.play("error"); return; }
        var finishAdd = function (res) {
          Audio.play("click"); UI.toast("Request sent to " + res.name, "gold");
          if (global.Net) Net.sendFriendRequestCrossTab(res.name, Store.get().profile.name);
          UI.pop(); setTimeout(friends, 280);
        };
        var local = Store.sendFriendRequest(name);
        if (local.ok) { finishAdd(local); return; }
        if (local.reason.indexOf("doesn't exist") === -1) { UI.toast(local.reason, "error"); Audio.play("error"); return; }
        // not on this device — verify against the cloud
        if (!global.Supabase || !Supabase.configured || !Supabase.isReady()) { UI.toast("Can't check the cloud — try again when connected.", "error"); Audio.play("error"); return; }
        Supabase.findPlayer(name, function (found) {
          if (!found) { UI.toast(name + " doesn't exist.", "error"); Audio.play("error"); return; }
          Supabase.sendFriendRequestCloud(found, function (ok) {
            if (!ok) { UI.toast("Could not send the request — check your connection.", "error"); Audio.play("error"); return; }
            var res = Store.sendFriendRequestCloud(found.username, found);
            finishAdd(res);
          });
        });
      } })
      ...
      (s.friendIncoming || []).forEach(function (req) {
        var row = UI.el("div", { class: "friend-row" }, [
          ...
          UI.button("ACCEPT", { sm: true, primary: true, onclick: function () {
            ...
            if (req.cloud && req.reqId) { Supabase.respondFriendRequest(req.reqId, true, function (ok) { if (ok) proceed(); else UI.toast("Could not reach the cloud.", "error"); }); }
            else proceed();
          } }),
          UI.button("DECLINE", { sm: true, danger: true, onclick: function () {
            ...
            if (req.cloud && req.reqId) { Supabase.respondFriendRequest(req.reqId, false, function (ok) { if (ok) proceed(); else UI.toast("Could not reach the cloud.", "error"); }); }
            else proceed();
          } })
        ]);
    ...
    if (pendingOut) {
      ...
      s.friendOutgoing.forEach(function (req) {
        var row = UI.el("div", { class: "friend-row" }, [
          H.avatarNode({ avatar: (req.avatar || req.name.slice(0, 2)) }, 44),
          ...
          UI.button("CANCEL", { sm: true, danger: true, onclick: function () {
            ...
            if (req.cloud && req.reqId) { Supabase.deleteFriendRequest(req.reqId, function (ok) { if (ok) proceed(); else UI.toast("Could not reach the cloud.", "error"); }); }
            else proceed();
          } })
        ]);
        scr._body.appendChild(row);
    ...
    UI.push(scr);
    syncCloudFriends(false, function (fetched) {
      if (fetched) { UI.pop(); setTimeout(friends, 250); }
    });
  }
  ...
  function settings() {
    ...
    card.appendChild(row("Log Out", "Return to the sign-in screen", UI.button("LOG OUT", { navy: true, sm: true, onclick: function () { UI.confirm("Log out of Loutris?", function () { if (global.Supabase && Supabase.configured) Supabase.signOut(function () {}); Store.patch(function (s) { s.authed = false; s.authMethod = null; }); UI.popAll(); setTimeout(function () { AuthScreen.show(); }, 300); }, { title: "LOG OUT?", yesLabel: "LOG OUT" }); } })));

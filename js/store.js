...
(function (global) {
  ...
  var PATREON_URL = "https://patreon.com/Loutris?utm_medium=unknown&utm_source=join_link&utm_campaign=creatorshare_creator&utm_content=copyLink";
  var lsOk = (function () {
  ...
  function save() {
    ...
    if (lsOk) { try { window.localStorage.setItem(KEY, s); } catch (e) {} }
    else { memStore = s; }
    // background cloud sync (Supabase) — debounced, best-effort
    try { if (global.Supabase && global.Supabase.markDirty) global.Supabase.markDirty(); } catch (e) {}
  }
  ...
  // Adopt a state blob (e.g. pulled from Supabase) as the live state.
  function replace(blob) {
    if (!blob || typeof blob !== "object") return;
    var clean = JSON.parse(JSON.stringify(blob));
    delete clean._dbUpdatedAt;
    state = mergeDefaults(clean, defaultState());
    save();
    notify();
  }

  // Merge only server-owned fields into the existing local shell. Local
  // cosmetics, friends, history, and offline settings are never uploaded as
  // progression and are not discarded when a cloud snapshot arrives.
  function applyCloudSnapshot(snapshot) {
    if (!snapshot || typeof snapshot !== "object") return;
    patch(function (s) {
      var p = snapshot.profile || {}, r = snapshot.ranked || {}, st = snapshot.stats || {}, c = snapshot.currency || {};
      s.authed = true;
      s.authMethod = snapshot.authMethod || s.authMethod || "email";
      if (p.name) s.profile.name = p.name;
      if (p.avatar) s.profile.avatar = p.avatar;
      if (p.country !== undefined) s.profile.country = p.country;
      if (p.team !== undefined) s.profile.team = p.team;
      if (p.level != null) s.profile.level = p.level;
      if (p.xp != null) s.profile.xp = p.xp;
      if (p.totalXp != null) s.profile.totalXp = p.totalXp;
      Object.keys(r).forEach(function (key) { if (key !== "rankedPlayed" && key !== "rankedWins") s.ranked[key] = r[key]; });
      if (r.rankedPlayed != null) s.stats.rankedPlayed = r.rankedPlayed;
      if (r.rankedWins != null) s.ranked.rankedWins = r.rankedWins;
      ["wins", "losses", "draws", "currentStreak", "bestStreak", "matchesPlayed"].forEach(function (key) { if (st[key] != null) s.stats[key] = st[key]; });
      if (c.coins != null) s.currency.coins = c.coins;
      if (c.gems != null) s.currency.gems = c.gems;
      if (snapshot.settings) s.settings = mergeDefaults(snapshot.settings, s.settings);
    });
  }
  ...
  function addCurrency(cur, amount) {
    if (!Object.prototype.hasOwnProperty.call(CUR, cur) || !Number.isFinite(amount) || amount <= 0) return false;
    patch(function (s) { s.currency[cur] = (s.currency[cur] || 0) + Math.floor(amount); });
    return true;
  }
  function spendCurrency(cur, amount) {
    ...
    patch(function (st) { st.currency[cur] -= Math.floor(amount); });
    return true;
  }
  ...
  function recordMatch(result) {
    ...
    // Every 10 matches, ask for support with a Patreon promo overlay.
    if (get().stats.matchesPlayed % 10 === 0) showSupportPromo();
  }
  function showSupportPromo() {
    if (!global.UI) return;
    // Let the match result settle first, then pop the promo.
    setTimeout(function () {
      var UI = global.UI;
      var card = UI.el("div", { class: "promo-card" }, [
        UI.el("div", { class: "promo-crown", html: '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M5 16l2.5-9 3.5 6 3.5-6 2.5 9H5z"/><circle cx="7.5" cy="5" r="1.4"/><circle cx="12" cy="3" r="1.6"/><circle cx="16.5" cy="5" r="1.4"/></svg>' }),
        UI.el("div", { class: "promo-kicker", text: "10 GAMES DONE — THANK YOU!" }),
        UI.el("div", { class: "promo-title", text: "SUPPORT THE HOUSE" }),
        UI.el("div", { class: "promo-desc", text: "Support me by becoming a Patreon. it means alot :>" }),
        UI.el("div", { class: "promo-actions" }, [
          UI.button("BECOME A PATREON", { primary: true, onclick: function () { window.open(PATREON_URL, "_blank", "noopener"); } }),
          UI.button("MAYBE LATER", { ghost: true, onclick: function () { veil.remove(); } })
        ])
      ]);
      var veil = UI.veil(card, { cls: "promo-veil" });
      if (global.Audio) Audio.play("reward");
    }, 1200);
  }
  ...
  // Friend request for a player verified against the cloud (no local registry).
  function sendFriendRequestCloud(name, info) {
    var s = get();
    if (name === (s.profile.name || "").toUpperCase()) return { ok: false, reason: "You can't add yourself." };
    if (_hasName(s, name)) return { ok: false, reason: name + " is already in your list or pending." };
    var id = Math.random().toString(36).slice(2);
    var elo = (info && info.elo) || 300;
    var avatar = (info && info.avatar) || name.slice(0, 2);
    patch(function (st) {
      st.friendOutgoing.push({ id: id, name: name, when: Date.now(), elo: elo, avatar: avatar });
      _addToRegistry(st, { name: name, elo: elo, avatar: avatar, status: "offline" });
    });
    return { ok: true, name: name, id: id };
  }
  ...
  function acceptFriendRequest(id) {
    patch(function (s) {
      ...
      if (!s.friends.some(function (f) { return f.name === req.name; })) {
        ...
        var elo = (req.elo != null) ? req.elo : (reg ? reg.elo : 300);
        var avatar = req.avatar || (reg ? reg.avatar : req.name.slice(0, 2));
  ...
  global.Store = {
    load: load, save: save, get: get, patch: patch, reset: reset, replace: replace, applyCloudSnapshot: applyCloudSnapshot, subscribe: subscribe,
    ...
    showSupportPromo: showSupportPromo,

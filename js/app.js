...
(function (global) {
  ...
  // ---- cloud sync (Supabase): pull once, adopt if newer, else push ----
  function syncCloud() {
    if (!global.Supabase || !Supabase.configured) return;
    Supabase.init(function (uid) {
      if (!uid) return;
      Supabase.pull(function (db) {
        if (!db) { refreshHomeIfAuthed(); return; }
        if (Store.applyCloudSnapshot) Store.applyCloudSnapshot(db);
        else Store.replace(db);
        var ns = Store.get();
        if (!ns.authed) { AuthScreen.show(); }
        else { AuthScreen.hide(); onAuthed(); }
      });
    });
  }

  function refreshHomeIfAuthed() {
    if (Store.get().authed) refreshHome();
  }
  ...
  function renderHomeLeaderboard(s) {
    ...
    var paint = function (rows) {
      ...
      rows = rows.slice(0, 3);
      var me = rows.filter(function (row) { return row.me; })[0];
      if (me && rows.indexOf(me) === -1) rows.push(me);
      if (!rows.length) {
        box.appendChild(UI.el("div", { class: "mute", style: { "padding": "10px 4px", "font-size": "11px" }, text: "No players yet. Play a match to appear on the ladder." }));
        return;
      }
      rows.forEach(function (r) {
        ...
        box.appendChild(UI.el("div", { class: cls }, [
          ...
          UI.el("div", { class: "lb-name" }, [UI.el("span", { text: r.name }), r.me ? UI.el("span", { class: "you", text: "YOU" }) : null]),
          UI.el("div", { class: "lb-elo", text: String(r.elo) })
        ]));
      });
    };
    // Real cloud ladder only — never local/fake data.
    if (global.Supabase && Supabase.configured && Supabase.isReady()) {
      var myName = (s.profile.name || "").toUpperCase();
      Supabase.leaderboard("global", function (rows) {
        ...
        if (hasMe) { paint(rows); return; }
        Supabase.myStanding("global", function (mine) {
      ...
      UI.clear(box);
      box.appendChild(UI.el("div", { class: "mute", style: { "padding": "10px 4px", "font-size": "11px" }, text: "Offline — reconnect to load the ladder." }));
    }
  }
  ...
  function renderTeamWar(s) {
    ...
    // totals: cloud (every player on every device) with local registry fallback
    var paint = function (info) {
    ...
    if (global.Supabase && Supabase.configured && Supabase.isReady()) {
      Supabase.teamStats(function (ts) {
        if (ts) {
          paint({ blue: ts.blue.elo, red: ts.red.elo, counts: { blue: ts.blue.count, red: ts.red.count } });
          return;
        }
        paint(Store.teamElo(s));
      });
    } else {
      paint(Store.teamElo(s));
    }
  ...
  function startHomeTimers() {
    ...
    // live cloud panels (leaderboard + team war totals) — only when online
    homeTimers.push(setInterval(function () {
      if (!global.Supabase || !Supabase.configured || !Supabase.isReady()) return;
      var s = Store.get();
      if (s.authed) { renderHomeLeaderboard(s); renderTeamWar(s); }
    }, 45000));
  }
  ...
  function wireHomeOnce() {
    ...
    on("#nav-battlepass", "click", function () {
      if (global.BattlePassScreen && BattlePassScreen.open) BattlePassScreen.open();
      else UI.toast("Battle Pass is unavailable.", "error");
      activateNav("nav-battlepass");
    });
    ...
    on("#btn-patreon", "click", function () { window.open(Store.PATREON_URL, "_blank", "noopener"); });
  ...
  window.addEventListener("error", function () {
    try { if (global.Supabase && Supabase.lastError && Supabase.lastError()) Store.pushNotification("A recoverable app error occurred.", "error"); } catch (e) {}
  });
  window.addEventListener("unhandledrejection", function () {
    try { if (global.Supabase && Supabase.lastError && Supabase.lastError()) Store.pushNotification("A cloud request failed. Please try again.", "error"); } catch (e) {}
  });
